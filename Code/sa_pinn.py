...
@jit
def loss(params, X, Y, lam, bound, bfilter, ref):
    """
    ...
    Args:
        ...
        Y (Tracer of DeviceArray): An array of shape (n, d) representing n collocation points in 
        d-dimensional space.
        lam (Tracer of DeviceArray): The lambda regularisation parameter for the boundary loss.
...
@jit
def step(istep, opt_state, X, Y, opt_state_lam, bound, bfilter, ref):
    """
    ...
    Args:
        ...
        Y (Tracer of DeviceArray): An array of shape (n, d) representing n collocation points in 
        d-dimensional space.
        opt_state_lam (Tracer of OptimizerState): The OptimizerState instance containing the 
    ...
    g = grad(loss, argnums=0, has_aux=True)(params, X, Y, lam, bound, bfilter, ref)
    g_lam = grad(loss, argnums=3, has_aux=True)(params, X, Y, lam, bound, bfilter, ref)
    return opt_update(istep, g[0], opt_state), opt_update_lam(
        istep, -g_lam[0], opt_state_lam
    )
...
params = init_network_params(layer_sizes, random.PRNGKey(96413))
lam = random.uniform(random.PRNGKey(73589), shape=[1])
...
start = time.time()
for it in pbar:
    opt_state, opt_state_lam = step(
        it, opt_state, x, y, opt_state_lam, bound, bfilter, ref
    )
    if it % 1 == 0:
        params = get_params(opt_state)
        lam = get_params_lam(opt_state_lam)[0]
        loss_full, losses = loss(params, x, y, lam, bound, bfilter, ref)
        l_b = losses[1]
        l_f = losses[0]
        l2 = losses[2]
...
axs[0].set_title("SA-PINN Proposed Solution")
axs[0].set_xlabel("x")
axs[0].set_ylabel("y")
