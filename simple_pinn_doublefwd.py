...
from jax import grad, jit, vmap, jacfwd, jacrev, jvp
...
def net_ff(params):
    ...
    def uxx(X, Y):
        return jacfwd(jacrev(net_u, argnums=1), argnums=1)(params, X, Y)
    
    def uyy(X,Y):
        return jacfwd(jacrev(net_u, argnums=2), argnums=2)(params, X, Y)
...
def net_bigu(params, X, Y):
    return vmap(vmap(net_u, (None, 0, None)), (None, None, 0))(params, X, Y)

@jit
def net_laplace(params, X, Y):
    v = jnp.ones(X.shape)
    u_xx = hvp_fwdfwd(lambda x: net_bigu(params, x, Y), (X,), (v,))
    u_yy = hvp_fwdfwd(lambda y: net_bigu(params, X, y), (Y,), (v,))
    return u_xx + u_yy
